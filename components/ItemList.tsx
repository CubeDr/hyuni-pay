...
const ItemRow: React.FC<{ item: Item; payers: Payer[]; onTogglePayer: (itemId: string, payerId: string) => void; onToggleShared: (itemId: string) => void; }> = ({ item, payers, onTogglePayer, onToggleShared }) => {
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          ...
          {!item.isShared && payers.map((payer, index) => {
            ...
            return (
              <button
                key={payer.id}
                onClick={() => onTogglePayer(item.id, payer.id)}
                title={payer.name}
...
const ItemList: React.FC<ItemListProps> = ({ items, payers, onTogglePayer, onToggleShared }) => {
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold">Itemized List</h2>
      <div className="space-y-3">
        {items.map(item => (
          <ItemRow key={item.id} item={item} payers={payers} onTogglePayer={onTogglePayer} onToggleShared={onToggleShared}/>
        ))}
      </div>
    </div>
  );
